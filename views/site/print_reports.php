    ...
    <div class="card border-0 shadow-sm rounded-3 overflow-hidden bg-white">
        ...
        <div class="card-header bg-primary text-white py-3 d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-2">
            ...
            <div class="d-flex align-items-center gap-2">
                <a href="<?= Url::toRoute(['site/batch-print-reports', 'class_level' => $selectedClass, 'term' => $selectedTerm]) ?>" class="btn btn-sm btn-light text-primary fw-bold text-xs d-flex align-items-center gap-1 shadow-sm" target="_blank">
                    <i class="bi bi-printer-fill"></i> Batch Print Cleared Cards (<?= Html::encode($selectedTerm) ?>)
                </a>
                
                <a href="<?= Url::toRoute(['site/export-class-marks', 'class_level' => $selectedClass, 'term' => $selectedTerm]) ?>" class="btn btn-sm btn-success text-white fw-bold text-xs d-flex align-items-center gap-1 shadow-sm">
